...
for month in months:
    ...
    if track_file.is_file() is False:
        ...
        files     = list(savedir.glob(('features_tb*' +  str(year) + str(month).zfill(2) + '??.nc' )))
        files_opt = list(savedir.glob(('features_tb*' +  str(year) + str(month).zfill(2) + '*_opt.nc' )))
        files_dc  = list(savedir.glob(('features_tb*' +  str(year) + str(month).zfill(2) + '*_dc.nc' )))
        files.sort()
        files_opt.sort()
        files_dc.sort()
        print(len(files), len(files_opt), len(files_dc))
        ...
        for idx in np.arange(len(files)):
            ...
            df = xr.open_dataset(files_opt[idx]).to_dataframe()
            df['feature'] = df.index.values
            features_df_list_opt.append(df)
        ...
        features_opt = tobac.utils.combine_tobac_feats(features_df_list_opt)
        ...
        # reduce tracks to valid cells and those cells that contain deep convection
        tracks_opt = tracks_opt[tracks_opt.cell != -1]
    ...
    if track_file.is_file() is True:
        ...
        for day in days:
            ...
            fname = Path(savedir / ('features_storm_tracks_tb_' +  str(year) + str(month).zfill(2) + str(day).zfill(2) + '_dc.nc'))
