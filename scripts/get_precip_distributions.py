...
output_dir = path / 'precip_features'
...
for day in days:
    ...
    daily_files.sort()
    mrms= xr.open_dataset(daily_files[0])
    
    for fname in daily_files[1:]:
            mrms = xr.concat([mrms, xr.open_dataset(fname)], dim = 'time') 
    ...
    ### Read in track/feature and mask file ###
    mask_tb = xr.open_dataset(path / 'tb' / subdir /  ('mask_tb_2020' + month + day +'_opt.nc'       ))
    mask_iwp = xr.open_dataset(path / 'iwp' / subdir / ('mask_iwp_2020' + month + day + '.nc' ))
    tracks_tb = xr.open_dataset(path / 'tb' / subdir / ('features_tb_2020' + month + day + '_opt.nc'   )).to_dataframe()
    tracks_iwp = xr.open_dataset(path / 'iwp' / subdir / ('features_iwp_2020' +month+ day + '.nc'  )).to_dataframe()
    ...
    if np.unique(precip_type.time.size) != mask_iwp_conus.time.size:
        print(precip_type.time.size, mask_iwp_conus.time.size)
        continue
    else:
        ...
        tracks_iwp_conus_xr.to_netcdf( output_dir / ('iwp_mrms_precip_stats_2020' + month  + day + '.nc'))
        ...
        tracks_tb_conus_xr.to_netcdf( output_dir / ('tb_mrms_precip_stats_2020' +  month +  day + '_opt.nc'))  
